...
function power_surge(){
    ...
    if (power_surge_status) {
        ...
        color_temp[0] = Math.round(color_temp[0] * 1000) / 1000;  
        color_temp[1] = Math.round(color_temp[1] * 1000) / 1000;
        color_temp[2] = Math.round(color_temp[2] * 1000) / 1000;
...
function adjust_projection_matrix(){
    temp_gaze = projectionMatrix;
    ...
    if(focused_gaze_status){
        ...
        console.log(ytop, bottom);

        if(ytop == 9 || bottom == -9 || ytop == 3 || bottom == -3){
            focused_gaze_toggle *= -1;
        }
        
        requestAnimationFrame(adjust_projection_matrix);
    }
    else{
        return;
    }
