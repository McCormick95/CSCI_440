...
var switch_middle= false;
var switch_right= false;
var switch_reset= false;
        ...
        switch( event.target.index ) {
			case 0: {
             switch_left= !switch_left;
                ...
                if (!switch_left) {
                    leftColor   = vec3(1.0, 0.0, 0.0);
                } else {
                    leftColor   = vec3(1.0, 1.0, 1.0);
                    }
			...
			case 1: {
             switch_middle= !switch_middle;

                if (!switch_middle) {
                    middleColor   = vec3(0.0, 1.0, 0.0);
                } else {
                    middleColor   = vec3(1.0, 1.0, 1.0);
                    }
                    gl.uniform4fv(gl.getUniformLocation(program, "diffuseProduct"), flatten(mult(lightDiffuse, materialDiffuse)));
			...
			case 2: {
             switch_right= !switch_right;
                if (!switch_right) {
                    rightColor   = vec3(0.0, 0.0, 1.0);
                } else {
                    rightColor   = vec3(1.0, 1.0, 1.0);
                    }
                    gl.uniform4fv(gl.getUniformLocation(program, "diffuseProduct"), flatten(mult(lightDiffuse, materialDiffuse)));
